...
async function getPath(path: string, options: EtaConfig): Promise<string> {
  ...
  if (match && match.length) {
    ...
    includePath = await searchViews(views, formattedPath);
    if (!includePath) {
    ...
    if (!includePath) {
      includePath = await searchViews(views, path);
    }
    if (!includePath) {
      throw EtaErr(
